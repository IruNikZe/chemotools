...
def slodget_cho_banded(cb: np.ndarray, lower: bool) -> tuple[float, float]:
    """Computes the logarithm of the absolute value of the determinant of a banded
    ...
    Parameters
    ----------
    cb : np.ndarray of shape (m, n)
        A NumPy-2D-Array resembling the Cholesky-decomposition of the matrix under
        consideration in banded storage format as returned by ``cholesky_banded``.
        Since ``cholesky_banded`` would fail for non-positive-definite matrices, this
        function does not need to check for this.
    lower : bool
        Whether the Cholesky-decomposition resulted from the lower or the upper
        triangular form of the matrix under consideration.
    ...
    return 1.0, 2.0 * np.sum(np.log(cb[main_diag_idx, ::]))
